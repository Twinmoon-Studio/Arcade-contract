...
contract ArcadeVault is Initializable, UUPSUpgradeable, OwnableUpgradeable{
   ...
   function updateSupplies(TOKEN_CHOICE _t, uint256 _amount) external onlyManager returns (bool _r) {
       if (_t == TOKEN_CHOICE.BONK) {
           supplies.bonkSupply += _amount;
           return true;
       } else if (_t == TOKEN_CHOICE.USDT) {
           supplies.usdtSupply += _amount;
           return true;
       }
       return false;
   }
   ...
   function withdrawFromReserve(TOKEN_CHOICE _t, address _u, uint256 _a) external onlyManager {
       ...
       if (_t == TOKEN_CHOICE.BONK && _a <= bonkReserve) {
           assets.bonkToken.safeTransfer(_u, _a);
           reserves.bonkReserve -= _a;
           supplies.bonkSupply -= _a;
       } else if (_t == TOKEN_CHOICE.USDT && _a <= usdtReserve) {
           assets.usdtToken.safeTransfer(_u,_a);
           reserves.usdtReserve -= _a;
           supplies.usdtSupply -= _a;
       } else {
           revert("Cannot Withdraw");
       }
   ...
   function withdraw(TOKEN_CHOICE _t, address _u, uint256 _a) external onlyOwner {
        ...
        if (_t == TOKEN_CHOICE.BONK) {
            assets.bonkToken.safeTransfer(_u,_a);
            supplies.bonkSupply -= _a;
            reserves.bonkReserve = 0;
        } else if (_t == TOKEN_CHOICE.USDT) {
            assets.usdtToken.safeTransfer(_u,_a);
            supplies.usdtSupply -= _a;
            reserves.usdtReserve = 0;
        } else {
            revert("Cannot withdraw");
        }
   }
